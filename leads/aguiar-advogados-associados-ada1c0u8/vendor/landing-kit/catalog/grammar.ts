...
export function grammarViolations(
  ...
  if (!brief.hasRating) {
    for (const section of spec.sections) {
      ...
      if (family === 'testimonials' || section.component === 'social-proof.numbers') {
...
function heroAllowList(brief: GrammarBrief): ComponentId[] {
  const lite: ComponentId[] = brief.hasPhotos
    ? ['hero.split-image', 'hero.centered', 'hero.full-image', 'hero.gradient']
    : ['hero.gradient', 'hero.centered', 'hero.morphing'];
...
export function allowedComponentsForType(
  ...
  const defaults: Record<string, ComponentId[]> = {
    header: ['navbar.minimal', 'navbar.centered', 'navbar.premium'],
    hero: heroAllowList(brief),
    services: ['features.cards', 'features.bento', 'features.alternating'],
    about: brief.hasPhotos
      ? ['about.split', 'about.editorial']
      : ['about.editorial'],
    gallery: ['gallery.grid', 'gallery.masonry'],
    testimonials: brief.hasRating
      ? ['social-proof.numbers', 'testimonials.cards', 'testimonials.quotes']
      : [],
    faq: ['faq.accordion'],
    cta: ['cta.banner', 'cta.split'],
