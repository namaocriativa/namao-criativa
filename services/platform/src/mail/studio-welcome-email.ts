...
export function studioWelcomeEmailHtml(params: {
  ...
  const reset = params.kind === 'reset';
  const heading = reset
    ? `Senha nova do studio, ${name}.`
    : `Bem-vindo ao studio, ${name}.`;
  const intro = reset
    ? 'Geramos uma nova senha para você entrar na plataforma. Guarde estes dados e use o botão abaixo.'
    : 'Criamos seu acesso à plataforma. Guarde o e-mail e a senha abaixo e entre quando quiser.';
...
<html lang="pt-BR">
  <head>
    ...
    <title>Acesso ao studio — Namão Criativa</title>
  </head>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
            ...
            <tr>
              <td style="border:1px solid rgba(255,255,255,0.12);padding:32px 28px 28px;">
                <p style="margin:0 0 8px;font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#8c8c8c;">Studio</p>
                <h1 style="margin:0 0 12px;font-family:Syne,Georgia,sans-serif;font-size:28px;line-height:1.1;letter-spacing:-0.04em;color:#f2f2f2;">${heading}</h1>
                ...
                <p style="margin:24px 0 0;text-align:center;">
                  <a href="${loginUrl}" style="display:inline-block;background:#f2f2f2;color:#050505;text-decoration:none;font-weight:700;padding:14px 22px;">Entrar no studio</a>
                </p>
...
export function studioWelcomeEmailText(params: {
  name: string;
  email: string;
  password: string;
  loginUrl: string;
  ...
  return [
    ...
    '',
    reset
      ? 'Sua senha do studio da Namão Criativa foi redefinida:'
      : 'Seu acesso ao studio da Namão Criativa:',
    `E-mail: ${params.email}`,
