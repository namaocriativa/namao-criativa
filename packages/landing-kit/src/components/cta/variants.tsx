...
export function CtaDark({ id, props }: { id: string; props: CtaBandProps }) {
  if (!props.cta) return null;
  return (
    <section className="lk-section lk-cta lk-cta--dark" id={id}>
      <div className="lk-container">
        <Heading title={props.title} lead={props.subtitle} />
        <CtaButton cta={props.cta} className="lk-btn lk-btn--invert lk-btn--xl" />
        {props.footnote ? <p className="lk-cta__footnote">{props.footnote}</p> : null}
      </div>
    </section>
  );
}
